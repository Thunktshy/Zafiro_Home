...
PedidosRouter.post('/remove_item', requireAuth, async (req, res) => {
  try {
    ...
    const params = {
      pedido_id:   { type: sql.NVarChar(20), value: p.pedido_id },
      producto_id: { type: sql.NVarChar(20), value: p.producto_id },
      cantidad:    { type: sql.Int,          value: p.cantidad ?? null }
    };
...
PedidosRouter.post('/set_estado', requireAuth, async (req, res) => {
  try {
    ...
    // devuelve encabezado + detalles
    const header = await db.executeProc('pedidos_get_by_id', { id: { type: sql.NVarChar(20), value: p.pedido_id } });
    const detalles = await db.executeProc('pedidos_join_detalles_by_pedido', { id_pedido: { type: sql.NVarChar(20), value: p.pedido_id } });
    return res.status(200).json({ success: true, message: 'Estado actualizado', data: { pedido: header?.[0] || null, detalles } });
  } catch (err) {
...
PedidosRouter.get('/verificar_productos/:pedido_id', requireAuth, async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: data.length ? 'Productos con stock insuficiente' : 'Todos con stock suficiente', data });
  } catch (err) {
