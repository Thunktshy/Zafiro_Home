...
// Helper para construir parámetros MSSQL
...
// Convierte a Date si es posible
const parseDT = (v) => {
...
const ensureRange = (d, h) => {
  if (d && h && d > h) {
    const e = new Error('Rango inválido');
    e.status = 400;
    throw e;
  }
};
...
ReportesRouter.get('/ventas_mensual_pivot', requireAdmin, async (req, res) => {
  try {
    const d = parseDT(req.query.desde),
      h = parseDT(req.query.hasta);
    ensureRange(d, h);
...
ReportesRouter.get('/historial_cliente', requireAdmin, async (req, res) => {
  try {
    ...
    if (!cliente_id) {
      return res
        .status(400)
        .json({ success: false, message: 'cliente_id requerido' });
    }
    // Normaliza el formato del cliente_id a 'cl-#'
    if (!cliente_id.startsWith('cl-')) {
